...
class ImageConverterApp:
    ...
    def create_widgets(self):
        ...
        # Разделение верхней части на две половины: Настройки и Предпросмотр HTML
        ...
        lbl_source = tk.Label(frame_source, text="Файлы:", width=15, anchor='w')  # Установлена фиксированная ширина
        ...
        lbl_output_folder = tk.Label(frame_output, text="Папка:", width=15, anchor='w')  # Установлена фиксированная ширина
        ...
        self.text_html_preview = tk.Text(frame_right, height=15, width=60, state='disabled', wrap='none')  # wrap='none' отключает переносы
    ...
    def update_preview(self):
        ...
        # Добавляем все генерируемые файлы
        ...
        # Добавляем "code.txt" только один раз и в конец списка
        if self.generate_html.get():
            ...
            if code_path not in self.file_to_item:
                ...
                self.file_to_item[code_path] = item_id  # Сохраняем ID элемента
    ...
    def generate_html_preview_for_first_image(self, source_path):
        ...
        # Определяем src как самый маленький (наименьшая ширина)
        if smallest_image:
            src = smallest_image[1]
        else:
            src = srcset_entries[0].split(" ")[0]

        srcset_str = ",\n\t".join(srcset_entries)  # Один знак табуляции перед каждым srcset

        # Форматируем HTML-код согласно примеру с точным количеством табуляций
        # Перед alt, class, loading, sizes, src - два знака табуляции
        # Перед srcset entries - один знак табуляции
        html_code = f'''<img
\talt="{alt_text}"
\tclass="profile-image"
\t{loading_attr}
...
">'''
        ...
        # Добавляем или обновляем HTML-код в предпросмотре
    ...
    def convert_images_thread(self):
        ...
        total_files = len(source_paths) * len(widths) * len(selected_formats_list)
        processed_files = 0
                ...
                with Image.open(source_path) as img:
                    ...
                    for width in widths:
                        ...
                        for fmt in selected_formats_list:
                            ...
                            # Увеличиваем счётчик обработанных файлов и обновляем прогрессбар
                            processed_files += 1
                    ...
                    if self.generate_html.get():
                        for base_name, files in generated_files_current.items():
                            # Сортируем файлы по ширине и формату
                            sorted_files = sorted(files, key=lambda x: (x[0], selected_formats_list.index(os.path.splitext(x[1])[1][1:].upper().replace('JPG','JPEG'))))
...
">'''
        ...
        if self.generate_html.get() and html_code_full.strip():
            ...
            try:
                ...
                # Обновляем статус "code.txt" в Treeview
    ...
    def update_progress_bar(self, increment):
        ...
        # Если прогрессбара ещё нет, создадим его
        if not hasattr(self, 'progress_bar'):
            ...
            # Установим максимум как общее количество файлов
